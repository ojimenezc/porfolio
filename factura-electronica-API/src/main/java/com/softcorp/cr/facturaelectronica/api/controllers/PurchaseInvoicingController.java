...
public class PurchaseInvoicingController extends BaseController {
    protected Logger logger = LoggerFactory.getLogger(PurchaseInvoicingController.class);
    @Autowired
    ...
    public GenerateDocumentResponse create(@RequestBody BuyBillGenerationRequest request) throws IOException, SAXException, ParserConfigurationException {
        ...
        try {
            ...
            logger.error("The document number " + (null != facturaElectronica ? facturaElectronica.getClave() : "") + " was not successfully sent to hacienda. Error cause = " + badRequest.getResponseHeaders().get("X-Error-Cause"), badRequest);
