...
public class TicketsController extends BaseController {
    ...
    public GenerateDocumentResponse create(@RequestBody TicketsRequest request) throws IOException, SAXException, ParserConfigurationException {

        URI location = null;
        GenerateDocumentResponse generateDocumentResponse = new GenerateDocumentResponse();
        TiqueteElectronico tiqueteElectronico = null;
        ...
        try {
            ...
            tiqueteElectronico = request.getDocument();

            File temp = File.createTempFile(tiqueteElectronico.getClave(), ".xml");
            String tempPath = temp.getPath();

            String xmlUnsigned = tempPath;
            Document sourceDoc = xmlHelper.getDocument(tiqueteElectronico, TiqueteElectronico.class);
            ...
            base64Xml = signer.sign(companyInformation.get(),
                    "https://cdn.comprobanteselectronicos.go.cr/xml-schemas/v4.3/tiqueteElectronico", tempPath);

            DocumentSendRequest sendRequest = new DocumentSendRequest();
            sendRequest.setClave(tiqueteElectronico.getClave());

            EmisorReceptorForPostRequest emisor = new EmisorReceptorForPostRequest();
            emisor.setNumeroIdentificacion(tiqueteElectronico.getEmisor().getIdentificacion().getNumero());
            emisor.setTipoIdentificacion(tiqueteElectronico.getEmisor().getIdentificacion().getTipo());
            sendRequest.setEmisor(emisor);

            EmisorReceptorForPostRequest receptor = new EmisorReceptorForPostRequest();
            receptor.setNumeroIdentificacion(tiqueteElectronico.getReceptor().getIdentificacion().getNumero());
            receptor.setTipoIdentificacion(tiqueteElectronico.getReceptor().getIdentificacion().getTipo());
            sendRequest.setReceptor(receptor);

            sendRequest.setComprobanteXml(base64Xml);
            sendRequest.setFecha(tiqueteElectronico.getFechaEmision().toGregorianCalendar().toInstant().toString());
            ...
            logGeneratedDocumentInfo(location, tiqueteElectronico.getClave(), logger);
            ...
            currentConsecutive.setConsecutive(tiqueteElectronico.getNumeroConsecutivo());
            currentConsecutive.setDocumentKey(tiqueteElectronico.getClave());
            ...
            String json = mapper.writeValueAsString(tiqueteElectronico);
            jsonBase64 = Base64.getEncoder().encodeToString(json.getBytes());
            documentsHelper.saveDocument(request.getCompanyId(), tiqueteElectronico.getClave(), jsonBase64, base64Xml);

        } catch (HttpClientErrorException.BadRequest badRequest) {
            logger.error("The document number " + (null != tiqueteElectronico ? tiqueteElectronico.getClave() : "") + " was not successfully sent to hacienda. Error cause = " + badRequest.getResponseHeaders().get("X-Error-Cause"), badRequest);
